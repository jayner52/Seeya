-- Update create_trip_with_locations to persist arrival_date and departure_date
-- for each location (collected in the wizard's Where & When step).
...
BEGIN
  ...
  -- 3. Add locations (now includes arrival_date / departure_date)
  IF p_locations IS NOT NULL AND jsonb_array_length(p_locations) > 0 THEN
    INSERT INTO public.trip_locations (trip_id, custom_location, order_index, arrival_date, departure_date)
    SELECT
      v_trip_id,
      loc->>'custom_location',
      (loc->>'order_index')::integer,
      (loc->>'arrival_date')::date,
      (loc->>'departure_date')::date
