-- SECURITY DEFINER RPC function for creating a trip with locations and participants.
-- This bypasses all RLS on all three tables (trips, trip_participants, trip_locations)
-- so client-side RLS policy tangles can never block trip creation.
-- Security is enforced inside the function: we check auth.uid() explicitly.
